...
def speckle(image):
    ...
    noise = np.random.normal(loc=0.0, scale=var[idx], size=image.shape).astype(np.float32)
    noisy_image = np.clip(image + image * noise, 0, 1)
    return (noisy_image * 255).astype(np.uint8), idx+1
...
def write_range_to_hdf5(counter_from, counter_to, images_data, labels_data, diagnoses_data, ids_data, ood_data,
                        quality_data, images, labels, ids, diagnoses, ood, quality):
    """ writes range of 4 images to hdf5 file
                    ...
                    ood_data       hdf5 dataset
                    quality_data       hdf5 dataset
                    images         list of images as numpy arrays
                    labels         list of labels as numpy arrays
                    ids            list of ids as strings
                    diagnosis      list of diagnosis as strings
                    ood            list of whether ood or not in int
                    quality        list of quality in int
    """
    # add images
    ...
    # add ood
    ood_data[counter_from:counter_to] = np.asarray(ood, dtype=int)

    # add quality
    quality_data[counter_from:counter_to] = np.asarray(quality, dtype=int)


def add_images(image_paths, images_data, labels_data, diagnoses_data, ids_data, ood_data, quality_data, counter_from, diagnosis):
    ...
    for i in range(len(image_paths)):
        ...
        if write_buffer >= max_write_buffer:
            ...
            write_range_to_hdf5(counter_from, counter_to, images_data, labels_data, diagnoses_data, ids_data, ood_data,
                                quality_data, images, labels, ids, diagnoses, ood, quality)
            # delete cash/lists
            images.clear()
            labels.clear()
        ...
        numeric_part = ''.join(filter(str.isdigit, img_id))
        result = int(numeric_part)  # convert the numeric part to an integer
        quality.append(quality_file["quality"][result - 1])

        image = cv2.imread(image_paths[i], cv2.IMREAD_COLOR)
        image = clahe_equalized(image)

        # apply ood transformation to a few images
        if random.random() > 0.7:
            image, score = speckle(image)
            ood.append(score)
        else:
            ood.append(0)
...
h5_file_path = "FIVES_experiment.h5"
...
path_to_train = "/mnt/qb/baumgartner/rawdata/FIVES/train"
...
path_to_test = "/mnt/qb/baumgartner/rawdata/FIVES/test"
# sort by diagnosis
...
# (>=1 of 3 quality features are marked as 0)
...
# Set 'quality' to 0 if at least one element is 0, otherwise set to 1
quality_file["quality"] = count_zeros
...
ood_cal = h5_file.create_dataset("cal/ood", shape=(number_cal_images), dtype=int)
ood_test = h5_file.create_dataset("test/ood", shape=(number_test_images), dtype=int)

quality_cal = h5_file.create_dataset("cal/quality", shape=(number_cal_images), dtype=int)
quality_test = h5_file.create_dataset("test/quality", shape=(number_test_images), dtype=int)

# ---- cal ----
add_images(A_cal, img_data_cal, label_data_cal, diagnosis_data_cal, id_cal, ood_cal, quality_cal, 0, "A")
add_images(G_cal, img_data_cal, label_data_cal, diagnosis_data_cal, id_cal, ood_cal, quality_cal, len(A_cal), "G")
add_images(N_cal, img_data_cal, label_data_cal, diagnosis_data_cal, id_cal, ood_cal, quality_cal, len(A_cal)+len(G_cal), "N")
add_images(D_cal, img_data_cal, label_data_cal, diagnosis_data_cal, id_cal, ood_cal, quality_cal, len(A_cal)+len(G_cal)+len(N_cal), "D")

# ---- test ----
add_images(A_test, img_data_test, label_data_test, diagnosis_data_test, id_test, ood_test, quality_test, 0, "A")
add_images(G_test, img_data_test, label_data_test, diagnosis_data_test, id_test, ood_test, quality_test, len(A_test), "G")
add_images(N_test, img_data_test, label_data_test, diagnosis_data_test, id_test, ood_test, quality_test, len(A_test)+len(G_test), "N")
add_images(D_test, img_data_test, label_data_test, diagnosis_data_test, id_test, ood_test, quality_test, len(A_test)+len(G_test)+len(N_test), "D")
