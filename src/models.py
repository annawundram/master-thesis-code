...
from src.augmentation import MixUp
...
from src.metrics import per_label_dice, generalised_energy_distance, predict_dice
...
class PHISeg(AbstractHierarchicalProbabilisticSegmentationModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        samples_s = s(y_pred_samples)
        samples_s = samples_s.detach().cpu().numpy()
        mean_dice_batch, var_dice_batch = predict_dice(samples_s)
        pred_dice_batch = torch.from_numpy(np.asarray(mean_dice_batch).copy())
        pred_dice = torch.mean(pred_dice_batch, dim=0)
        ...
        # log true squared error only if dice is sufficient
        if dice >= 0.7:
            self.log("val/squared error pp", (dice - pred_dice)**2, on_epoch=True)
        else:
            self.log("val/squared error pp", 100, on_epoch=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test/dice", torch.mean(dice), on_epoch=True)
...
class UNet(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        s = torch.nn.Softmax(dim=1)
        samples_s = s(y_hat)
        samples_s = torch.unsqueeze(samples_s, dim=1)
        samples_s = samples_s.detach().cpu().numpy()  # 4x1x2x320x320
        mean_dice_batch, var_dice_batch = predict_dice(samples_s)
        pred_dice_batch = torch.from_numpy(np.asarray(mean_dice_batch).copy())
        pred_dice = torch.mean(pred_dice_batch, dim=0)
...
class UNetMCDropout(UNet):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        s = torch.nn.Softmax(dim=1)
        samples_s = s(y_pred_samples)
        samples_s = samples_s.detach().cpu().numpy()  # 4x10x2x320x320
        mean_dice_batch, var_dice_batch = predict_dice(samples_s)
        pred_dice_batch = torch.from_numpy(np.asarray(mean_dice_batch).copy())
        pred_dice = torch.mean(pred_dice_batch, dim=0)
